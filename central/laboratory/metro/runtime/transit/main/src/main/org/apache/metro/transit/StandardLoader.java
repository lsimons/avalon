...
public class StandardLoader implements Repository
{
     ...
     */
     public StandardLoader( InitialContext context )
     {
        if( null == context ) 
    ...
    */
       ...
       throws IOException
    {
        ...
        PluginDescriptor descriptor = getPluginDescriptor( artifact );
    ...
    * @return the plugin class
    */
    private Class getPluginClass( ClassLoader parent, PluginDescriptor descriptor ) 
       throws IOException
    {
    ...
    * @exception RepositoryException if a factory creation error occurs
    */
    private PluginDescriptor getPluginDescriptor( Artifact artifact )
      throws IOException
    {
        if( null == artifact ) throw new NullPointerException( "artifact" );
        ...
        try
        {
            ...
            return new PluginDescriptor( attributes );
        }
        catch( CacheException ce )
        {
            final String error = 
    ...
    */
    private Object createPlugin( 
      ClassLoader classloader, PluginDescriptor descriptor, Class clazz, Object[] args )
      throws IOException
    {
        ...
        for( int i=0; i<arguments.length; i++ )
        {
            if( null == arguments[i] )
            {
                ...
                else if( PluginDescriptor.class.isAssignableFrom( c ) )
                {
     ...
     */
    private ClassLoader createClassLoader( ClassLoader base, PluginDescriptor descriptor )
        throws IOException
    {
        if( null == descriptor ) 
        ...
        URL[] apis = 
          getURLs( 
            descriptor.getDependencies( PluginDescriptor.API_KEY ) );
        ClassLoader api = buildClassLoader( base, apis );

        URL[] spis = 
          getURLs( 
            descriptor.getDependencies( PluginDescriptor.SPI_KEY ) );
        ClassLoader spi = buildClassLoader( api, spis );

        URL[] imps = 
          getURLs( artifact, descriptor.getDependencies( PluginDescriptor.IMP_KEY ) );

        return buildClassLoader( spi, imps );
    }
